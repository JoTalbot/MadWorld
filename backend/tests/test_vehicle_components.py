...
from app.domain.primitives import DamageType, InvalidTransition, Vehicle, VehicleState
...
def test_component_repair_restores_durability() -> None:
    ...
    assert restored == 20
    assert vehicle.components["engine"].condition == 75
    assert vehicle.durability == 94
