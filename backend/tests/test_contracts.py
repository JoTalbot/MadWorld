...
from app.domain.contracts import ContractObjective, ContractReward, ContractRisk, ContractTemplate, ContractState
...
def test_contract_progression_deadline_and_deterministic_reward():
    player=uuid4(); wallet_id=uuid4(); template_id=uuid4(); now=datetime(2026,1,1,tzinfo=timezone.utc)
...
def test_contract_prerequisite_and_expiry():
    player=uuid4(); a=uuid4(); b=uuid4(); now=datetime(2026,1,1,tzinfo=timezone.utc); uow=InMemoryUnitOfWork(); uow.contracts=InMemoryContractRepository(); service=ContractService(uow)
