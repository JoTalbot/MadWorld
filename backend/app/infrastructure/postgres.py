...
class PostgresJobRepository:
    ...
    def save(self, job: Job) -> None:
        if job.version == 0:
            try: self.conn.execute(text("INSERT INTO jobs (id, owner_id, job_type, state, started_at, completes_at, idempotency_key, version) VALUES (:id, :owner_id, :job_type, :state, :started_at, :completes_at, :key, 1)"), {"id": job.id, "owner_id": job.owner_id, "job_type": job.job_type, "state": job.state.value, "started_at": job.started_at, "completes_at": job.completes_at, "key": f"pending:{job.id}"}); job.version = 1; return
            except IntegrityError as exc: raise map_integrity_error(exc) from exc
...
class PostgresUnitOfWork(UnitOfWork):
    ...
    def __enter__(self) -> "PostgresUnitOfWork":
        self.conn = self.engine.connect(); self.transaction = self.conn.begin(); self.wallets = PostgresWalletRepository(self.conn); self.inventories = PostgresInventoryRepository(self.conn); self.jobs = PostgresJobRepository(self.conn); self.characters = PostgresCharacterRepository(self.conn); self.vehicles = PostgresVehicleRepository(self.conn); self.idempotency = PostgresIdempotencyRepository(self.conn); self.audit = PostgresAuditRepository(self.conn); self.outbox = PostgresOutboxRepository(self.conn); return self
