...
class PostgresJobRepository:
    ...
    def get_by_idempotency_key(self, key: str) -> Job | None:
        row = self.conn.execute(text("SELECT id, owner_id, job_type, state, started_at, completes_at, version, metadata FROM jobs WHERE idempotency_key = :key"), {"idempotency_key": key}).mappings().first(); return self._map(row) if row else None
