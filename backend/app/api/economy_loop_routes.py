...
def _next_action(active_jobs: int, contract_count: int, ready_vehicles: int, market_points: int) -> str:
    if active_jobs: return "job_in_progress"
    if contract_count: return "review_contracts"
    if ready_vehicles: return "prepare_expedition"
    if market_points: return "review_market"
...
def economy_overview(authenticated_player: UUID = Depends(get_authenticated_player)) -> EconomyOverview:
    with get_engine().connect() as conn:
        settlement = conn.execute(
            text("SELECT settlement_id, region, level FROM player_settlement_state WHERE player_id=:player"),
            {"player": authenticated_player},
        ).mappings().first()
        if settlement is None: raise ValueError("player settlement not found")
        settlement_id = UUID(str(settlement["settlement_id"]))
    ...
    return EconomyOverview(
        settlement_id=settlement_id, region=str(settlement["region"]), warehouse_capacity=capacity, warehouse_used=used,
        ...
        contract_count=contract_count, ready_vehicles=ready_vehicles, market_price_points=market_points,
