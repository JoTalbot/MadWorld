...
def create_session(payload: SessionCreateRequest) -> SessionResponse:
    ...
    with get_engine().begin() as conn:
        row = conn.execute(
            text("SELECT id, handle FROM players WHERE handle = :handle"),
            {"handle": payload.handle},
        ).mappings().first()
        if row is None:
            row = conn.execute(
                text("INSERT INTO players (handle) VALUES (:handle) RETURNING id, handle"),
                {"handle": payload.handle},
            ).mappings().one()

        conn.execute(
            text("""
                INSERT INTO player_sessions (player_id, token_hash, created_at, last_seen_at, expires_at)
