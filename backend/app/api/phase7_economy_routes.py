...
def warehouse_create(payload: CapacityRequest, authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().begin() as conn:
        return dict(create_warehouse(conn, authenticated_player, payload.region_id, payload.name, payload.capacity_units))
...
def facility_create(payload: FacilityRequest, authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().begin() as conn:
        return dict(create_facility(conn, authenticated_player, payload.region_id, payload.name, payload.facility_type, payload.capacity_units))

@router.get("/production/recipes")
def production_recipes(authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().connect() as conn:
        rows = conn.execute(text("SELECT * FROM economy_recipes WHERE enabled=TRUE ORDER BY code")).mappings().all()
        return [dict(r) for r in rows]

@router.get("/production/jobs")
def production_jobs(authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().connect() as conn:
        rows = conn.execute(text("SELECT * FROM production_jobs WHERE owner_id=:o ORDER BY started_at DESC"), {"o": authenticated_player}).mappings().all()
        return [dict(r) for r in rows]

@router.post("/production/start", status_code=201)
def production_start(payload: ProductionRequest, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), authenticated_player: UUID = Depends(get_authenticated_player)):
    if not idempotency_key:
        raise ValueError("Idempotency-Key header is required")
    with get_engine().begin() as conn:
        return dict(start_production(conn, authenticated_player, payload.facility_id, payload.recipe_id, payload.batch_units, idempotency_key))

@router.post("/production/{job_id}/complete")
def production_complete(job_id: UUID, authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().begin() as conn:
...
def logistics(authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().connect() as conn:
        rows = conn.execute(text("SELECT * FROM logistics_contracts WHERE owner_id=:o ORDER BY created_at DESC"), {"o": authenticated_player}).mappings().all()
        return [dict(r) for r in rows]

@router.post("/logistics", status_code=201)
def logistics_create(payload: LogisticsRequest, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), authenticated_player: UUID = Depends(get_authenticated_player)):
    if not idempotency_key:
        raise ValueError("Idempotency-Key header is required")
    with get_engine().begin() as conn:
        return dict(create_logistics(conn, authenticated_player, payload.source_warehouse_id, payload.destination_warehouse_id, payload.item_definition_id, payload.quantity, payload.reward, payload.route_risk_bps, idempotency_key))

@router.post("/logistics/{contract_id}/deliver")
def logistics_deliver(contract_id: UUID, authenticated_player: UUID = Depends(get_authenticated_player)):
    with get_engine().begin() as conn:
