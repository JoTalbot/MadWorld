...
class VehicleComponent:
    ...
    def __post_init__(self) -> None:
        if not 0 <= self.condition <= self.max_condition <= 100: raise InvalidQuantity("component condition must be between 0 and max condition")
        if not 0 <= self.armor <= 100: raise InvalidQuantity("component armor must be between 0 and 100")
    @property
    def destroyed(self) -> bool: return self.condition <= 0
    def damage(self, raw_amount: int, damage_type: DamageType) -> int:
        if raw_amount <= 0: raise InvalidQuantity("damage amount must be positive")
        multipliers = {DamageType.KINETIC: 1.0, DamageType.EXPLOSIVE: 1.15, DamageType.FIRE: 0.9, DamageType.IMPACT: 0.8}
        effective = max(1, round(raw_amount * multipliers[damage_type] * (100 - self.armor) / 100))
        applied = min(self.condition, effective); self.condition -= applied; return applied
    def repair(self, amount: int) -> int:
        if amount <= 0: raise InvalidQuantity("repair amount must be positive")
        restored = min(amount, self.max_condition - self.condition); self.condition += restored; return restored
...
class Wallet:
    ...
    def credit(self, amount: int) -> None:
        if amount <= 0: raise InvalidQuantity("credit amount must be positive")
        self.balance += amount
...
class Job:
    ...
    def start(self) -> None:
        if self.state is not JobState.QUEUED: raise InvalidTransition("only queued jobs can start")
        self.state = JobState.RUNNING
    def complete(self, now: datetime) -> None:
        if self.state not in (JobState.QUEUED, JobState.RUNNING): raise InvalidTransition("only active jobs can complete")
        if now < self.completes_at: raise InvalidTransition("job completion time has not been reached")
...
class Vehicle:
    ...
    def repair(self, amount: int) -> None:
        ...
        for component in self.components.values():
            ...
            remaining -= component.repair(remaining)
        self._sync_durability()
    def repair_component(self, component: str, amount: int) -> int:
        if component not in self.components: raise ValueError(f"unknown vehicle component: {component}")
        restored = self.components[component].repair(amount); self._sync_durability(); return restored
    def apply_damage(self, component: str, amount: int, damage_type: DamageType) -> int:
        if self.state is VehicleState.DESTROYED: raise InvalidTransition("destroyed vehicle cannot take further damage")
        if component not in self.components: raise ValueError(f"unknown vehicle component: {component}")
        applied = self.components[component].damage(amount, damage_type); self._sync_durability()
        if all(c.destroyed for c in self.components.values()) or self.durability <= 0: self.state = VehicleState.DESTROYED
        return applied
    def component_effects(self) -> dict[str, float]:
        engine = self.components["engine"].condition / 100; wheels = self.components["wheels"].condition / 100; fuel = self.components["fuel_system"].condition / 100
        return {"engine_power": round(max(0.0, engine), 3), "mobility": round(max(0.0, wheels), 3), "fuel_efficiency": round(max(0.25, fuel), 3)}
    def _sync_durability(self) -> None:
        if not self.components: return
        total = sum(c.condition for c in self.components.values()); maximum = sum(c.max_condition for c in self.components.values())
        self.durability = max(0, min(100, round(total * 100 / maximum)))
    def refuel(self, amount: int) -> None:
        if amount <= 0: raise InvalidQuantity("refuel amount must be positive")
        self.fuel += amount
