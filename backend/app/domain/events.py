...
class EventEnvelope:
    """Stable transport envelope for authoritative domain events."""
    ...
    def to_dict(self) -> dict:
        ...
        value["aggregate_id"] = str(self.aggregate_id)
...
class EventSchemaRegistry:
    """In-process registry; storage and consumers depend on stable versions, not Python classes."""
    ...
    def register(self, event_type: str, schema_version: int, validator: EventValidator) -> None:
        if not event_type or schema_version < 1:
            raise ValueError("event type and schema version must be valid")
        key = (event_type, schema_version)
        if key in self._validators:
            raise ValueError(f"event schema already registered: {event_type} v{schema_version}")
    ...
    def validate(self, event_type: str, schema_version: int, payload: dict) -> None:
        ...
        if validator is None:
            raise ValueError(f"unknown event schema: {event_type} v{schema_version}")
        validator(payload)
...
def build_default_registry() -> EventSchemaRegistry:
    ...
    registry.register("job.created", 1, _require_keys("job_type"))
    registry.register("job.started", 1, _require_keys("state"))
    registry.register("job.completed", 1, _require_keys("state"))
    registry.register("job.cancelled", 1, _require_keys("state"))
    return registry
