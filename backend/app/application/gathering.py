...
class GatheringService:
    ...
    def _nodes(self):
        ...
        if conn is None:
            raise RuntimeError("resource node repository is not configured")
    ...
    def gather(self, player_id: UUID, inventory_id: UUID, node_id: UUID, now: datetime | None = None) -> tuple[ResourceNode, int]:
        ...
        node = self._nodes().get_for_update(node_id)
        if node is None:
        ...
        updated = ResourceNode(node.id, node.region_id, node.resource_item_definition_id,
                               node.quantity - amount, node.gather_amount, node.cooldown_seconds,
                               node.version, current)
        self._nodes().save(updated)
        InventoryService(self.uow).add(inventory_id, node.resource_item_definition_id, amount, 100)
