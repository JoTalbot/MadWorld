...
class JobService:
    ...
    def create(self, owner_id: UUID, job_type: str, started_at: datetime, completes_at: datetime, idempotency_key: str) -> Job:
        ...
        if existing is not None:
            if (existing.owner_id, existing.job_type, existing.started_at, existing.completes_at) != (owner_id, job_type, started_at, completes_at): raise IdempotencyConflict("idempotency key already belongs to another job")
            return existing
        job = Job.create(owner_id, job_type, started_at, completes_at); self.uow.jobs.save(job); self.uow.jobs.bind_idempotency_key(idempotency_key, job.id); self._record("job.created", job.id, {"job_type": job_type}); return job
