...
class ExpeditionService:
    ...
    def start(
        ...
        if existing is not None:
            if existing.job_type != f"{EXPEDITION_JOB_PREFIX}{region}" or existing.metadata.get("vehicle_id") != str(vehicle_id):
        ...
        seed = f"{player_id}:{vehicle_id}:{region}:{distance_km}:{risk.value}:{idempotency_key}".encode()
        digest = hashlib.sha256(seed).digest()
        damage = int(digest[0] % (8 + risk.value.__len__() * 6))
        loot_scrap = 5 + digest[1] % (6 + distance_km // 20)
        loot_wire = digest[2] % (1 + distance_km // 25)
        ...
        metadata = {
            ...
            "damage_type": DamageType.IMPACT.value,
            "loot_scrap": int(loot_scrap),
            "loot_wire": int(loot_wire),
            "resolved": False,
        }
    ...
    def complete(self, player_id: UUID, job_id: UUID, now: datetime | None = None):
        ...
        if int(metadata["loot_scrap"]) > 0:
            InventoryService(self.uow).add(UUID(metadata["inventory_id"]), SCRAP_METAL_ID, int(metadata["loot_scrap"]), 100)
        if int(metadata["loot_wire"]) > 0:
            InventoryService(self.uow).add(UUID(metadata["inventory_id"]), SALVAGED_WIRE_ID, int(metadata["loot_wire"]), 100)
