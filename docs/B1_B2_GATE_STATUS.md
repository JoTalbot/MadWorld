...
- PostgreSQL integration coverage now exercises the persisted encounter-loss → travel-loss → vehicle-destruction → recovery chain, duplicate terminal requests, exactly-once recovery debit, and cargo-capacity boundary enforcement.
- Remaining gate item: exact-final-HEAD CI verification.
