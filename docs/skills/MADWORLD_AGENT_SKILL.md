...
### L8. Bound the complete Android smoke lifecycle; local fixtures are not device evidence
The release workflow at `df787dc` calls `adb wait-for-device` before entering its timeout. At 2026-09-08 12:23 UTC, four old Release Gate runs still had all 16 API smoke jobs in `Boot emulator`, while exact-head CI remained queued. The underlying emulator startup cause is not established by those job states. The older pending patch also left diagnostic `adb devices` unbounded and omitted `set -e` in its nested `bash -c`.

The replacement helper `ops/android_emulator_smoke.sh` is covered by fake-tool subprocess tests for shared boot deadlines, bounded adb/install/diagnostics, exact API checks, original failure propagation and owned-process cleanup. A regression test also reproduced and fixed a subtle false PASS: ignoring TERM inside an EXIT cleanup trap loses cancellation received during diagnostics. Preserve that cancellation in the final exit/result instead. These tests prove control flow only, never Android runtime compatibility or login/offline/reconnect coverage.

The refreshed patch is still a **pending delivery artifact**, not an applied workflow. `test_b10_release_workflow_contract.py` applies it to a disposable copy of the current workflow, fails on stale patches, then enforces the live workflow after the patch is applied and removed **in the same commit**. The unconditional live YAML guard remains active throughout. See `ops/B10_ANDROID_MATRIX_RECOVERY_2026-09-08.md` for evidence, the Actions cancellation permission blocker, and the exact owner handoff.

### L9. A pip-bundled PostgreSQL is not automatically a complete test environment
`pgserver==0.1.4` provides native PostgreSQL 16.2 binaries in this sandbox, but not the required `pgcrypto` extension. Its advertised `pgserver.POSTGRES_BIN_PATH` attribute is also absent; use the installed package directory's `pginstall/bin` and verify `postgres --version` / `pg_config`. The initial migration must fail when `pgcrypto` is missing — never weaken `001_foundation.sql`, register a stub extension, or call that failure an application regression. A genuine matching contrib extension was built and functionally verified for a private, Unix-socket-only fixture cluster; all 281 backend tests then passed with zero skips and all 43 migration checksums matched. This is local evidence only, not a recommended production distribution or a substitute for GitHub CI. Provenance, isolation, cleanup and reproduction boundaries: `ops/B10_POSTGRES16_VERIFICATION_2026-09-08.md`.

GitHub log-download errors can include temporary signed storage URLs in stderr. Sanitize before printing or saving errors; record the error class/hostname without query credentials. A successful redirect is not evidence that log content was retrieved. The Android log download in this continuation failed at the storage connection, so the emulator startup cause remains UNKNOWN.
