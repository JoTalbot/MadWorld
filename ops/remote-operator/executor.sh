...
execute_one() {
  ...
  if ! "$ROOT/state-manager.sh" claim "$state" "$id" "$attempt" "$EXECUTOR_ID" >/dev/null 2>&1; then return 0; fi
  ...
  rc=0; status=RUNNING; timed_out=0; cancel_requested=0
  set +e
...
PY
    then
      cancel_requested=1; rc=130; status=CANCELLED
    ...
    if (( $(date +%s) >= deadline )); then
      timed_out=1; rc=124; status=TIMEOUT
...
count_active() {
  local n=0 job
  for job in $(jobs -rp); do kill -0 "$job" 2>/dev/null && n=$((n+1)) || true; done
  printf '%s' "$n"
}
